...
from Polymer.src.PMPI import inp
...
class Chain:
    ...
    def check_border(self):
        if (abs(C_coord_x[-1] + self.x) < box_x / 2 and abs(C_coord_y[-1] + self.y) < box_y / 2 and abs(
                C_coord_z[-1] + self.z) < box_z / 2):
            return 0
        else:
            return 1
...
def create_tube(Diam, L, init_pos, anglex, angley):
    N = round(math.pi / math.sin(r / Diam))  # number of atoms in one circle
    ...
    for i in range(len(a[0])):
        a[0][i] = a_cp[0][0] + (a_cp[0][i] - a_cp[0][0]) * math.cos(anglex) - (a_cp[2][i] - a_cp[2][0]) * math.sin(
            anglex)
        a[2][i] = a_cp[2][0] + (a_cp[0][i] - a_cp[0][0]) * math.sin(anglex) + (a_cp[2][i] - a_cp[2][0]) * math.cos(
            anglex)
    ...
    for i in range(len(a[0])):
        a[1][i] = a_cp[1][0] + (a_cp[1][i] - a_cp[1][0]) * math.cos(angley) - (a_cp[2][i] - a_cp[2][0]) * math.sin(
            angley)
        a[2][i] = a_cp[2][0] + (a_cp[1][i] - a_cp[1][0]) * math.sin(angley) + (a_cp[2][i] - a_cp[2][0]) * math.cos(
            angley)
...
def main():
    ...
    for j in range(chain_numb):
        while 1:
            first_chain = Chain([random.randint(-box_x / 2, box_x / 2), random.randint(-box_y / 2, box_y / 2),
                                 random.randint(-box_z / 2, box_z / 2)])
            chain_len = 2
            for i in range(bead_number):
                ...
                while 1:
                    ...
                    if time.time() - start > time_to_wait:
                        break
                    if (
                            first_chain.check_angle() == 0 and first_chain.check_empty() == 0 and first_chain.check_border() == 0):
